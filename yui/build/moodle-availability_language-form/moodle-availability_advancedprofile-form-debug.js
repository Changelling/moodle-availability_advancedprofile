YUI.add('moodle-availability_advancedprofile-form', function (Y, NAME) {

    /**
     * JavaScript for form editing profile conditions.
     *
     * @module moodle-availability_advancedprofile-form
     */
    M.availability_advancedprofile = M.availability_advancedprofile || {};
    
    /**
     * @class M.availability_advancedprofile.form
     * @extends M.core_availability.plugin
     */
    M.availability_advancedprofile.form = Y.Object(M.core_availability.plugin);
     ...
     * @type Array
     */
    M.availability_advancedprofile.form.profiles = null;
     ...
     */
    M.availability_advancedprofile.form.initInner = function(standardFields, customFields) {
        this.standardFields = standardFields;
        this.customFields = customFields;
    };
    
    M.availability_advancedprofile.form.getNode = function(json) {
        // Create HTML structure.
        var html = '<span class="availability-group"><label><span class="pr-3">' +
                M.util.get_string('conditiontitle', 'availability_advancedprofile') + '</span> ' +
        ...
        html += '</select></label> <label><span class="accesshide">' + M.util.get_string('label_operator', 'availability_advancedprofile') +
                ' </span><select name="op" title="' + M.util.get_string('label_operator', 'availability_advancedprofile') + '"' +
                         ' class="custom-select">';
        var operators = ['isequalto', 'contains', 'doesnotcontain', 'startswith', 'endswith',
                'isempty', 'isnotempty'];
        for (i = 0; i < operators.length; i++) {
            html += '<option value="' + operators[i] + '">' +
                    M.util.get_string('op_' + operators[i], 'availability_advancedprofile') + '</option>';
        }
        html += '</select></label> <label><span class="accesshide">' + M.util.get_string('label_value', 'availability_advancedprofile') +
                '</span><input name="value" type="text" class="form-control" style="width: 10em" title="' +
                M.util.get_string('label_value', 'availability_advancedprofile') + '"/></label></span>';
        ...
        if (!M.availability_advancedprofile.form.addedEvents) {
            M.availability_advancedprofile.form.addedEvents = true;
            var updateForm = function(input) {
                var ancestorNode = input.ancestor('span.availability_advancedprofile');
            ...
            root.delegate('change', function() {
                 updateForm(this);
            }, '.availability_advancedprofile select');
            root.delegate('change', function() {
                 updateForm(this);
            }, '.availability_advancedprofile input[name=value]');
        }
    
        return node;
    };
    
    M.availability_advancedprofile.form.fillValue = function(value, node) {
        // Set field.
    ...
    M.availability_advancedprofile.form.fillErrors = function(errors, node) {
        var value = {};
        this.fillValue(value, node);
    
        // Check profile item id.
        if (value.sf === undefined && value.cf === undefined) {
            errors.push('availability_advancedprofile:error_selectfield');
        }
        if (value.v !== undefined && /^\s*$/.test(value.v)) {
            errors.push('availability_advancedprofile:error_setvalue');
        }
    };
