 ...
 *
 * @package availability_advancedprofile
 * @copyright 2014 The Open University
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

namespace availability_advancedprofile;
...
class condition extends \core_availability\condition {
    ...
    /** @var string Operator: field is bigger */
    const OP_IS_BIGGER = 'isbigger';
    ...
    public function __construct($structure) {
        ...
        if (isset($structure->op) && in_array($structure->op, array(self::OP_CONTAINS,
                ...
                self::OP_ENDS_WITH, self::OP_IS_EMPTY, self::OP_IS_NOT_EMPTY, 
                self::OP_IS_BIGGER, self::OP_IS_SMALLER), true)) {
    ...
    public function is_available($not, \core_availability\info $info, $grabthelot, $userid) {
        $uservalue = $this->get_cached_user_advancedprofile_field($userid);
    ...
    public function get_description($full, $not, \core_availability\info $info) {
        ...
        if ($this->customfield) {
            ...
            $customfields = self::get_custom_advancedprofile_fields();
            if (array_key_exists($this->customfield, $customfields)) {
                $translatedfieldname = $customfields[$this->customfield]->name;
            } else {
                $translatedfieldname = get_string('missing', 'availability_advancedprofile',
                        $this->customfield);
            }
        } else {
        ...
        return get_string('requires_' . $opname, 'availability_advancedprofile', $a);
    }
     ...
     */
    public static function get_custom_advancedprofile_fields() {
        global $DB, $CFG;
     ...
     */
    protected function get_cached_user_advancedprofile_field($userid) {
        global $USER, $DB, $CFG;
        ...
        if ($iscustomprofilefield) {
            ...
            if (!array_key_exists($this->customfield, self::get_custom_advancedprofile_fields())) {
    ...
    private function get_condition_sql($field, $field2 = null, $istext = false) {
        ...
        switch($this->operator) {
            ...
            case self::OP_IS_NOT_EMPTY:
                ...
                break;
            case self::OP_IS_BIGGER:                
                $sql = "(DATE(".$field.") >= DATE(".$this->value."))"
                break;
            case self::OP_IS_SMALLER:                
                $sql = "(DATE(".$field.") < DATE(".$this->value."))"
                break;
